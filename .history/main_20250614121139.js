...
// Navbar shrink and active link highlight on scroll
...
function animateCounters() {
    document.querySelectorAll('.stat-number, .stat-number-home').forEach(counter => {
        const target = parseInt(counter.dataset.count);
        let count = 0;
        const step = Math.ceil(target / 50);
        const plus = counter.classList.contains('stat-number') ? '+' : '';
...
const statsSection = document.querySelector('.tech-stats-home') || document.querySelector('.stats-section');
if (statsSection) observer1.observe(statsSection);
...
function createParticle() {
    ...
    particle.style.background = 'rgba(0,212,255,0.12)';
    particle.style.borderRadius = '50%';
    particle.style.pointerEvents = 'none';
    particle.style.opacity = 0.5;
    particle.style.transition = 'opacity 2s linear';
    particles.appendChild(particle);
    setTimeout(() => { particle.style.opacity = 0; }, 1000);
    setTimeout(() => { particles.removeChild(particle); }, 2000);
}
