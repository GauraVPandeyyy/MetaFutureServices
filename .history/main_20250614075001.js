(() => {
    ...
    if (window.gsap && window.ScrollTrigger) {
        ...
        gsap.to('.ceo-container', {
            ...
            scrollTrigger: { trigger: '.ceo-section', start: 'top 70%', toggleActions: 'play none none none' }
        });
    ...
    // Animate stats numbers with "+" after counting
    document.querySelectorAll('.stat-number').forEach(stat => {
        const rawText = stat.textContent.trim();
        const hasPlus = rawText.endsWith('+');
        const targetValue = parseInt(rawText.replace(/\D/g, ''));
        ...
        // Animate when in view (all screens)
        const observer = new IntersectionObserver(entries => {
            if (entries[0].isIntersecting) {
                animate();
                observer.disconnect();
    ...
    // Animate .stat-number-home (no plus)
    function animateStatNumberHome(stat) {
        const dataCount = stat.getAttribute('data-count');
        if (!dataCount) return;
        const targetValue = parseInt(dataCount, 10);
        let current = 0;
        const duration = 2000;
        const increment = targetValue / (duration / 16);
    ...
    // Observer for .stat-number-home
    document.querySelectorAll('.stat-number-home').forEach(stat => {
        const observer = new IntersectionObserver(entries => {
            if (entries[0].isIntersecting) {
                animateStatNumberHome(stat);
                observer.disconnect();
            }
        }, { threshold: 0.5 });
        observer.observe(stat);
    ...
    // === Animate all stat numbers (home + client stats) ===
    function animateStatNumber(stat) {
        const rawText = stat.textContent.trim();
        const dataCount = stat.getAttribute('data-count');
        if (!dataCount) return;

        // If value contains "/" (like 24/7), just show it, no animation
        if (rawText.includes('/')) {
    ...
    // Intersection Observer for all stat numbers
    function observeStatNumbers(selector) {
        document.querySelectorAll(selector).forEach(stat => {
            const observer = new IntersectionObserver(entries => {
                if (entries[0].isIntersecting) {
                    animateStatNumber(stat);
                    observer.disconnect();
    ...
    // Call for both home and client stats
    observeStatNumbers('.stat-number');
    observeStatNumbers('.stat-number-home');
})();
